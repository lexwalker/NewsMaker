...
def main() -> int:
    ...
    for row, c in classifications.items():
        ...
        # Col D — section
        data_updates.append(
            {"range": f"'{tab}'!D{row}", "values": [[section]]}
        )
        # Col E — region
        data_updates.append(
            {"range": f"'{tab}'!E{row}", "values": [[region]]}
        )
        # Col K — note
        data_updates.append(
            {"range": f"'{tab}'!K{row}", "values": [[note]]}
        )
        # Col M — verdict
        data_updates.append(
            {"range": f"'{tab}'!M{row}", "values": [[verdict]]}
        )
        # Col X — relevance
        data_updates.append(
            {"range": f"'{tab}'!X{row}", "values": [[relevance]]}
        )

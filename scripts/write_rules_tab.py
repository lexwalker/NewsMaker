...
def main() -> int:
    ...
    # ---- 6. Открытые вопросы ----
    add_section_header("6. Открытые вопросы (ещё не ответили)", "header_yellow")
    add_table_header("Вопрос", "Где смотреть", "Текущее поведение бота", "Нужно решение")
    table_header_rows.append(len(rows) - 1)
    open_qs = [
        ("Аналитика рынка / ежемесячные отчёты (Cox Weekly, ACEA, НАП)",
         "Уточнение у руководителя, группа 1",
         "LLM часто ставит Economics с 0.85–0.95, ставим как есть",
         "Включать ли целиком или фильтровать по доменам?"),
        ("Прогнозы / forecast (TrendForce EV cell prices)",
         "группа 2",
         "LLM ставит Economics/Other news",
         "Берём ли ВСЕ прогнозы? По частным случаям Д: «обычно постим»"),
        ("Whitepapers (Geotab blog)",
         "группа 3",
         "LLM ставит Other news с низкой confidence",
         "Как разделить «блог-пост бренда» от «новость»?"),
        ("Test-drive longform",
         "группа 4",
         "Помечаем колонкой «требует ручной проверки»",
         "Так оставляем или строго отсеиваем?"),
        ("Отзывы авто зарубежные (Transport Canada)",
         "группа 5",
         "LLM пропускает как возможные новости",
         "Брать ли отзывы по моделям, которые не продаются в РФ?"),
        ("Пресс-релизы дилерских/лизинговых компаний (Газпромбанк Автолизинг)",
         "группа 6",
         "LLM ставит Dealer news / Promo",
         "Нужны ли новости про сам лизинг, а не про модели?"),
        ("Ежегодные премии / NCAP winners / музеи",
         "группа 7",
         "LLM ставит Other news",
         "Ежегодные списки — берём каждый год или только один раз?"),
    ]
    for q, where, cur, need in open_qs:
        rows.append([q[:150], where, cur[:150], need[:200]])
